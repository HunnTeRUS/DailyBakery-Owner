...
module.exports = {
    ...
    async forgotPassword(request, response) {
        ...
        try {
            ...
            if (user) {
                ...
                return response
                    .status(404)
                    .json({ error: 'CNPJ invalido ou não encontrado' });
            }
        } catch (e) {
            return response.status(400).json({ error: e });
        }
    ...
    async updateAddress(request, response) {
        ...
        try {
            await Padaria.updateOne({ _id: _id }, {
                cep: cep,
                rua: rua,
                numero: numero,
                bairro: bairro,
    ...
    async getAddressByCep(request, response) {
        ...
        await Padaria.findOne({ _id: _id }).then((resp) => {
            if (!CoolDownVerify.CoolDown(resp.tempo_espera)) {
                return response
                    .status(406)
                    .json({ error: 'Endereço alterado recentemente' });
            }
        });
        await axios
            .get('https://www.cepaberto.com/api/v3/cep', {
                headers: {
                    Authorization: 'Token token=b8589b52d467c9c5ded3c65c244b4fe6',
                },
                params: { cep: cep },
            })
            .then((responseAPI) => {
    ...
    async getCnpjFromWs(request, response) {
        ...
        if (bakery) {
            return response
                .status(400)
                .json({ error: 'Já existe um cadastro dessa padaria em nossa base' });
        }
        axios
            ...
            .then((responseAPI) => {
                ...
                if (situacao == 'ATIVA') {
                    return response.status(200).json(responseAPI.data);
                }
                return response.status(406).json({
                    error: 'Infelizmente a padaria não pode ser cadastrada por estar com o status inativo na receita federal',
                });
            })
            .catch((erro) => {
    ...
    async verifyToken(request, response) {
        ...
        if (padarias) {
            ...
            response.status(400).json({
                error: 'Não foi possivel retornar a padaria a partir do token',
            });
    },

    async validateEmail(request, response) {
        const { email } = request.body;
        const valid_email = await Padaria.findOne({ email: email });

        if (valid_email) {
            return response
                .status(400)
                .json({ error: 'O e-mail informado já foi cadastrado em nossa base' });
        }
        return response.status(200).json({});
    },
};